...
$(document).ready(function () {
  ...
  $(".add-row-con").click(function () {
    var dumy_tr =
      ...
      product_con_count +
      '][test_report]" class="form-control"></textarea></td><td><input type="checkbox" class="record"></td></tr>';
  ...
  function destination(country_id) {
    $.ajax({
      method: "POST",
      url: base_url + "Gmark/get_destination",
      data: {
  ...
  function customer_list(class_set, selected) {
    $.ajax({
      ...
      success: function (data) {
        html = $.parseJSON(data);
        $(class_set).html('<option value="" >SELECT ONE</option>');
        $.each(html, function (index, value) {
          if (selected) {
        ...
        success: function (result) {
          ...
          if (data.errors) {
            ...
            $.each(error,function(i,v){
              $('#applicant_add input[name="'+i+'"]').after('<span class="text-danger add_content_report_error">'+v+'</span>');
              $('#applicant_add select[name="'+i+'"]').after('<span class="text-danger add_content_report_error">'+v+'</span>');
            });
          }else{
            $('.add_content_report_error').remove();
          }
    ...
    if (validation == true) {
      $.ajax({
        ...
        success: function (result) {
          ...
          if (data.errors) {
            $(".error_factory_Add").html(data.errors);
          } else {
            $(".error_factory_Add").html("");
          }
        },
        error: function () {
          iziToast.error({
  ...
  $("#regForm").on("submit", function (e) {
    ...
    if (form_submit == 1) {
      form_submit++;
      $.ajax({
        url: base_url + "Gmark/insert_gmark_registration",
        method: "POST",
        data: $(this).serialize(),
        success: function (result) {
          form_submit = 1;
